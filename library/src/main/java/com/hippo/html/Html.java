...
public class Html {
    ...
    private static String getTextDirection(Spanned text, int start, int end) {
        ...
        int paraDir = Reflections.bidi(2 /* Layout.DIR_REQUEST_DEFAULT_LTR */, buffer, levels, len,
                false /* no info */);
        switch(paraDir) {
        ...
        for (int i = start; i < end; i = next) {
            ...
            for (int j = 0; j < style.length; j++) {
                ...
                if (style[j] instanceof AbsoluteSizeSpan) {
                    ...
                    if (!s.getDip()) {
                        Application application = Reflections.currentApplication();
                        if (application != null) {
...
class HtmlToSpannedConverter implements ContentHandler {
    ...
    private static void startImg(Editable text, Attributes attributes, Html.ImageGetter img) {
        ...
        if (d == null) {
            d = Resources.getSystem().
                    getDrawable(Reflections.UNKNOWN_DRAWABLE_RES_ID);
    ...
    private int getHtmlColor(String color) throws NumberFormatException {
        return HtmlColor.parseColor(color.trim());
    }
